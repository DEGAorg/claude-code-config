...
- [x] Add already-running detection: if tmux session exists and engine window is alive, print status and exit. (deps: 1)
- [x] Add `orch_plan_log_file()` to orch-state.sh. Engine writes its log to `plans/<slug>/engine.log`. (deps: none)
