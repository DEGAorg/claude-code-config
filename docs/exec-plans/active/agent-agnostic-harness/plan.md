...
- [x] Update `scripts/planner-loop.sh` — source `agent-shim.sh`; replace `claude -p` invocations with shim helpers; replace `CLAUDE_SOUND` with `DEGA_SOUND`; replace `~/.claude/planner` with `$DEGA_CORE_HOME/state/planner/` (deps: 1)
