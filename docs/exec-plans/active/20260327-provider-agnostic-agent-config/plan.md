...
- [x] Update `commands/apply-core.md` — change `claude-md-template.md` refs to `agent-template.md`; install target remains `~/.claude/CLAUDE.md` (global config is provider-specific) (deps: 3)
- [x] Update `commands/core-init.md` — Step 6 writes `AGENTS.md` + shims (`CLAUDE.md`, `GEMINI.md`, `.cursorrules`) instead of just `CLAUDE.md` (deps: 3)
...
- [x] Verify: grep entire repo for stale `claude-md-template` references; verify project-level `CLAUDE.md` refs are shim-aware; run `shellcheck` on modified scripts (deps: 4, 5, 6, 7, 8, 9)
