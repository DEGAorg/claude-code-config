...
class SecretSetupTests(unittest.TestCase):
    def test_wallet_setup_protects_old_projects_and_preserves_examples(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            cli.chmod(0o755)
            for _ in range(2):
                subprocess.run(['bash', '-c', command], cwd=project,
                               env={**os.environ, 'DEGA_CORE_HOME': directory}, check=True)
            ...
            for name in ('.canon/wallet.env', '.env.local', 'strategies/demo/.env',
                         'strategies/demo/wallet.env'):
                result = subprocess.run(['git', 'check-ignore', '-q', name], cwd=project)
    ...
    def test_scaffold_ignores_wallet_before_it_exists(self):
        with tempfile.TemporaryDirectory() as directory:
            project = Path(directory)
            subprocess.run(['git', 'init', '-q', directory], check=True)
            (project / '.gitignore').write_bytes((ROOT / 'canon/templates/.gitignore').read_bytes())
            result = subprocess.run(['git', 'check-ignore', '-q', '.canon/wallet.env'],
                                    cwd=project)
