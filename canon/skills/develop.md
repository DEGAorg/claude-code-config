...
@description Implement, test, and iterate on a Canon strategy from a design specification until it passes QA. Triggers on natural-language intent like "develop the strategy", "build out the strategy from this spec", "implement and test this spec", or "write the code for this strategy". Expects a strategy spec path as context (from `/discover`, a `docs/strategy-*.md` file, or a user-provided spec). Do not invoke before a strategy spec exists (use `canon-start` to reach that phase first).
