...
@description Start or resume the Canon prediction-market pipeline on a project that already has Canon installed. Triggers on natural-language intent like "start canon", "run canon", "begin the canon pipeline", or "kick off canon here" — detects the project's current phase (init/scaffold/strategy/develop/run) and drives the full pipeline from that point. Do not invoke for brand-new projects that still need bootstrap (use `canon-new`) or for stopping a running strategy (use `canon-stop`).
