...
- Authentication: EOA-based by default. Canon auto-generates a project-local burner via `canon-cli wallet ensure` (stored at `.canon/wallet.env`, mode 0600) only during an explicitly requested `/canon-start --live` transition. Ordinary dry-run initialization does not create or fund a wallet. `POLYMARKET_PRIVATE_KEY` env var still wins when set (CI / bring-your-own-wallet). L2 API credentials are derived automatically from the EOA's L1 signature on first use. For order signing, `signatureType: "eoa"` (type 0). Gnosis Safe mode (type 2) is supported by pmxtjs but not wired into Canon yet.
