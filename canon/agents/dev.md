...
- Write tests and validate using `.canon/ralph.yaml` success criteria checks
...
- ALWAYS run all checks from `.canon/ralph.yaml` before considering implementation complete
...
2. Verify scaffold exists (`package.json`, `tsconfig.json`, `.canon/ralph.yaml`). If missing, tell user to run `/canon-init`
3. Implement strategy logic in src/strategy.ts
4. Implement RiskInterface in src/types/RiskInterface.ts
5. Write tests
6. Run all check commands from `.canon/ralph.yaml` `success_criteria` (`pnpm exec tsc --noEmit`, `pnpm exec oxlint src/`, `pnpm exec vitest run`)
...
- Check results from `.canon/ralph.yaml` success criteria
