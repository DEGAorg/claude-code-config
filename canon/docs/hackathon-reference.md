...
Build an AI trading bot for NBA Playoffs prediction markets. $1,000 prize. Free workshops. No experience needed.

The DEGA NBA Playoffs Prediction Market Hackathon is a global, fully virtual competition where developers build automated trading strategies for prediction markets — powered by Canon, DEGA's open-source CLI that scaffolds, runs, and monitors prediction market bots from the terminal.

Five free workshops take you from zero to running automation. Join at any time — no registration cutoff.
...
DEGA is building tools and infrastructure to empower developers to launch AI-powered prediction market automation. With a strong focus on developer experience and community-driven innovation, DEGA simplifies prediction market development through Canon — a CLI toolkit that lets developers bootstrap, configure, and deploy trading strategies from the terminal in minutes.
...
- Workshops and documentation guide you from zero to running automation.
- Join at any time — no registration cutoff.
...
Build AI-powered automations focused on prediction market microstructures:

- **Arbitrage Detection**: Exploit price discrepancies for the same event across different markets or within the same platform.
- **Cross-Market Analysis**: Correlate positions across related markets — when one moves, capitalize on the lag in another.
- **Speed-Based Opportunity**: Act on publicly available statistical data (team records, matchups, injury reports) before market prices adjust.

Canon's built-in templates focus on these microstructure-based approaches. Participants are free to explore other strategy types, but Canon does not provide templates for news sentiment or opinion-based strategies.
...
- Early invite to the upcoming World Cup Prediction Market Hackathon — a larger-scale competition with sponsor backing and a bigger prize pool.

## Why Join?

Walk away with a working prediction market bot, new skills in AI automation, and a portfolio project you can show off. The workshops take you from zero to running automation — and what you build here carries directly into the World Cup hackathon. Whether you're here to learn or to win, this is where it starts.
