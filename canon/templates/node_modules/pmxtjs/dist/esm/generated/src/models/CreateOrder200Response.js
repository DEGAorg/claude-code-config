/* tslint:disable */
/* eslint-disable */
/**
 * PMXT Sidecar API
 * A unified local sidecar API for prediction markets (Polymarket, Kalshi, Limitless). This API acts as a JSON-RPC-style gateway. Each endpoint corresponds to a specific method on the generic exchange implementation.
 *
 * The version of the OpenAPI document: 0.4.4
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
import { ErrorDetailFromJSON, ErrorDetailToJSON, } from './ErrorDetail.js';
import { OrderFromJSON, OrderToJSON, } from './Order.js';
/**
 * Check if a given object implements the CreateOrder200Response interface.
 */
export function instanceOfCreateOrder200Response(value) {
    return true;
}
export function CreateOrder200ResponseFromJSON(json) {
    return CreateOrder200ResponseFromJSONTyped(json, false);
}
export function CreateOrder200ResponseFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'success': json['success'] == null ? undefined : json['success'],
        'error': json['error'] == null ? undefined : ErrorDetailFromJSON(json['error']),
        'data': json['data'] == null ? undefined : OrderFromJSON(json['data']),
    };
}
export function CreateOrder200ResponseToJSON(json) {
    return CreateOrder200ResponseToJSONTyped(json, false);
}
export function CreateOrder200ResponseToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'success': value['success'],
        'error': ErrorDetailToJSON(value['error']),
        'data': OrderToJSON(value['data']),
    };
}
