"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Cache = exports.BET_ON_RACE_OUTCOME_SOL_DISCRIMINATOR = exports.PLACE_BET_SOL_DISCRIMINATOR = exports.BorshReader = exports.STATUS_NAMES = exports.MarketStatus = exports.LAMPORTS_PER_SOL = exports.RACE_POSITION_DISCRIMINATOR_BS58 = exports.USER_POSITION_DISCRIMINATOR_BS58 = exports.RACE_MARKET_DISCRIMINATOR_BS58 = exports.MARKET_DISCRIMINATOR_BS58 = exports.RACE_POSITION_DISCRIMINATOR = exports.USER_POSITION_DISCRIMINATOR = exports.RACE_MARKET_DISCRIMINATOR = exports.MARKET_DISCRIMINATOR = exports.PROGRAM_ID = void 0;
exports.parseMarket = parseMarket;
exports.parseRaceMarket = parseRaceMarket;
exports.parseUserPosition = parseUserPosition;
exports.parseRacePosition = parseRacePosition;
exports.mapBooleanToUnified = mapBooleanToUnified;
exports.mapRaceToUnified = mapRaceToUnified;
exports.deriveConfigPda = deriveConfigPda;
exports.deriveMarketPda = deriveMarketPda;
exports.derivePositionPda = derivePositionPda;
exports.deriveRaceMarketPda = deriveRaceMarketPda;
exports.deriveRacePositionPda = deriveRacePositionPda;
exports.getInstructionDiscriminator = getInstructionDiscriminator;
const web3_js_1 = require("@solana/web3.js");
const bs58_1 = __importDefault(require("bs58"));
const crypto_1 = require("crypto");
const market_utils_1 = require("../../utils/market-utils");
const price_1 = require("./price");
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
exports.PROGRAM_ID = new web3_js_1.PublicKey('FWyTPzm5cfJwRKzfkscxozatSxF6Qu78JQovQUwKPruJ');
// Anchor account discriminators (first 8 bytes of sha256("account:{AccountName}"))
exports.MARKET_DISCRIMINATOR = Buffer.from([219, 190, 213, 55, 0, 227, 198, 154]);
exports.RACE_MARKET_DISCRIMINATOR = Buffer.from([235, 196, 111, 75, 230, 113, 118, 238]);
exports.USER_POSITION_DISCRIMINATOR = Buffer.from([251, 248, 209, 245, 83, 234, 17, 27]);
exports.RACE_POSITION_DISCRIMINATOR = Buffer.from([44, 182, 16, 1, 230, 14, 174, 46]);
exports.MARKET_DISCRIMINATOR_BS58 = bs58_1.default.encode(exports.MARKET_DISCRIMINATOR);
exports.RACE_MARKET_DISCRIMINATOR_BS58 = bs58_1.default.encode(exports.RACE_MARKET_DISCRIMINATOR);
exports.USER_POSITION_DISCRIMINATOR_BS58 = bs58_1.default.encode(exports.USER_POSITION_DISCRIMINATOR);
exports.RACE_POSITION_DISCRIMINATOR_BS58 = bs58_1.default.encode(exports.RACE_POSITION_DISCRIMINATOR);
exports.LAMPORTS_PER_SOL = 1_000_000_000;
// Market status enum indices
exports.MarketStatus = {
    Active: 0,
    Closed: 1,
    Resolved: 2,
    Cancelled: 3,
    Paused: 4,
    ResolvedPending: 5,
    Disputed: 6,
};
exports.STATUS_NAMES = {
    0: 'active',
    1: 'closed',
    2: 'resolved',
    3: 'cancelled',
    4: 'paused',
    5: 'resolved_pending',
    6: 'disputed',
};
// ---------------------------------------------------------------------------
// Borsh Buffer Reader
// ---------------------------------------------------------------------------
class BorshReader {
    buf;
    offset;
    constructor(buf) {
        this.buf = Buffer.from(buf);
        this.offset = 0;
    }
    readU8() {
        const v = this.buf.readUInt8(this.offset);
        this.offset += 1;
        return v;
    }
    readU16() {
        const v = this.buf.readUInt16LE(this.offset);
        this.offset += 2;
        return v;
    }
    readU64() {
        const v = this.buf.readBigUInt64LE(this.offset);
        this.offset += 8;
        return v;
    }
    readI64() {
        const v = this.buf.readBigInt64LE(this.offset);
        this.offset += 8;
        return v;
    }
    readBool() {
        return this.readU8() === 1;
    }
    readPubkey() {
        const bytes = this.buf.subarray(this.offset, this.offset + 32);
        this.offset += 32;
        return bs58_1.default.encode(bytes);
    }
    readString() {
        const len = this.buf.readUInt32LE(this.offset);
        this.offset += 4;
        const str = this.buf.subarray(this.offset, this.offset + len).toString('utf8');
        this.offset += len;
        return str;
    }
    readOptionBool() {
        const hasValue = this.readU8();
        if (hasValue === 0)
            return null;
        return this.readBool();
    }
    readOptionU8() {
        const hasValue = this.readU8();
        if (hasValue === 0)
            return null;
        return this.readU8();
    }
    readOptionPubkey() {
        const hasValue = this.readU8();
        if (hasValue === 0)
            return null;
        return this.readPubkey();
    }
    readFixedBytes(len) {
        const bytes = this.buf.subarray(this.offset, this.offset + len);
        this.offset += len;
        return Buffer.from(bytes);
    }
    skip(len) {
        this.offset += len;
    }
    readOptionFixedBytes(len) {
        const hasValue = this.readU8();
        if (hasValue === 0)
            return null;
        return this.readFixedBytes(len);
    }
    getOffset() {
        return this.offset;
    }
}
exports.BorshReader = BorshReader;
// ---------------------------------------------------------------------------
// Account Parsers
// ---------------------------------------------------------------------------
function parseMarket(data) {
    const reader = new BorshReader(data);
    // Skip 8-byte discriminator
    reader.skip(8);
    const marketId = reader.readU64();
    const question = reader.readString();
    const closingTime = reader.readI64();
    const resolutionTime = reader.readI64();
    reader.readI64(); // auto_stop_buffer
    const yesPool = reader.readU64();
    const noPool = reader.readU64();
    reader.readU64(); // snapshot_yes_pool
    reader.readU64(); // snapshot_no_pool
    const status = reader.readU8();
    const winningOutcome = reader.readOptionBool();
    reader.readU8(); // currency_type
    reader.skip(33); // _reserved_usdc_vault
    reader.readU64(); // creator_bond
    reader.readU64(); // total_claimed
    reader.readU64(); // platform_fee_collected
    const lastBetTime = reader.readI64();
    reader.readU8(); // bump
    const layer = reader.readU8();
    reader.readU8(); // resolution_mode
    reader.readU8(); // access_gate
    const creator = reader.readPubkey();
    reader.readOptionPubkey(); // oracle_host
    reader.skip(160); // council [pubkey; 5]
    reader.skip(4); // council_size, council_votes_yes, council_votes_no, council_threshold
    reader.readU64(); // total_affiliate_fees
    reader.readOptionFixedBytes(32); // invite_hash
    const creatorFeeBps = reader.readU16();
    reader.readU64(); // total_creator_fees
    reader.readOptionPubkey(); // creator_profile
    const platformFeeBpsAtCreation = reader.readU16();
    reader.readU16(); // affiliate_fee_bps_at_creation
    reader.readI64(); // betting_freeze_seconds_at_creation
    const hasBets = reader.readBool();
    return {
        marketId,
        question,
        closingTime,
        resolutionTime,
        yesPool,
        noPool,
        status,
        winningOutcome,
        layer,
        creator,
        creatorFeeBps,
        platformFeeBpsAtCreation,
        hasBets,
        lastBetTime,
    };
}
function parseRaceMarket(data) {
    const reader = new BorshReader(data);
    // Skip 8-byte discriminator
    reader.skip(8);
    const marketId = reader.readU64();
    const question = reader.readString();
    const closingTime = reader.readI64();
    const resolutionTime = reader.readI64();
    reader.readI64(); // auto_stop_buffer
    const outcomeCount = reader.readU8();
    // outcome_labels: [[u8; 32]; 10]
    const outcomeLabels = [];
    for (let i = 0; i < 10; i++) {
        const labelBytes = reader.readFixedBytes(32);
        if (i < outcomeCount) {
            // Trim trailing zero bytes and decode as UTF-8
            const nonZero = Array.from(labelBytes).filter(b => b !== 0);
            outcomeLabels.push(Buffer.from(nonZero).toString('utf8'));
        }
    }
    // outcome_pools: [u64; 10]
    const outcomePools = [];
    for (let i = 0; i < 10; i++) {
        outcomePools.push(reader.readU64());
    }
    const totalPool = reader.readU64();
    // snapshot_pools: [u64; 10]
    reader.skip(80); // 10 * 8 bytes
    reader.readU64(); // snapshot_total
    const status = reader.readU8();
    const winningOutcome = reader.readOptionU8();
    reader.readU8(); // currency_type
    reader.readU64(); // platform_fee_collected
    reader.readU64(); // creator_fee_collected
    reader.readU64(); // total_claimed
    const lastBetTime = reader.readI64();
    reader.readU8(); // bump
    const layer = reader.readU8();
    reader.readU8(); // resolution_mode
    reader.readU8(); // access_gate
    const creator = reader.readPubkey();
    reader.readOptionPubkey(); // oracle_host
    reader.skip(160); // council [pubkey; 5]
    reader.readU8(); // council_size
    reader.skip(10); // council_votes [u8; 10]
    reader.readU8(); // council_threshold
    const creatorFeeBps = reader.readU16();
    reader.readOptionPubkey(); // creator_profile
    const platformFeeBpsAtCreation = reader.readU16();
    return {
        marketId,
        question,
        closingTime,
        resolutionTime,
        outcomeCount,
        outcomeLabels: outcomeLabels.slice(0, outcomeCount),
        outcomePools: outcomePools.slice(0, outcomeCount),
        totalPool,
        status,
        winningOutcome,
        layer,
        creator,
        creatorFeeBps,
        platformFeeBpsAtCreation,
        lastBetTime,
    };
}
function parseUserPosition(data) {
    const reader = new BorshReader(data);
    reader.skip(8); // discriminator
    const user = reader.readPubkey();
    const marketId = reader.readU64();
    const yesAmount = reader.readU64();
    const noAmount = reader.readU64();
    const claimed = reader.readBool();
    return { user, marketId, yesAmount, noAmount, claimed };
}
function parseRacePosition(data) {
    const reader = new BorshReader(data);
    reader.skip(8); // discriminator
    const user = reader.readPubkey();
    const marketId = reader.readU64();
    const bets = [];
    for (let i = 0; i < 10; i++) {
        bets.push(reader.readU64());
    }
    const totalBet = reader.readU64();
    const claimed = reader.readBool();
    return { user, marketId, bets, totalBet, claimed };
}
// ---------------------------------------------------------------------------
// Mapping to Unified Types
// ---------------------------------------------------------------------------
function mapBooleanToUnified(market, pubkey) {
    const totalPool = market.yesPool + market.noPool;
    const totalPoolSol = Number(totalPool) / exports.LAMPORTS_PER_SOL;
    // Implied probability from pool ratios (inverse — more bets on YES = higher NO implied price)
    // In pari-mutuel: your payout if YES wins = totalPool / yesPool
    // So the implied probability of YES = noPool / totalPool (complement side)
    // This matches the standard: price reflects how much you'd pay per unit of payout
    let yesPrice = 0.5;
    let noPrice = 0.5;
    if (totalPool > 0n) {
        yesPrice = Number(market.noPool) / Number(totalPool);
        noPrice = Number(market.yesPool) / Number(totalPool);
    }
    const outcomes = [
        {
            outcomeId: `${pubkey}-YES`,
            marketId: pubkey,
            label: 'Yes',
            price: (0, price_1.clampBaoziPrice)(yesPrice),
        },
        {
            outcomeId: `${pubkey}-NO`,
            marketId: pubkey,
            label: 'No',
            price: (0, price_1.clampBaoziPrice)(noPrice),
        },
    ];
    const um = {
        marketId: pubkey,
        title: market.question,
        description: '',
        outcomes,
        resolutionDate: new Date(Number(market.resolutionTime) * 1000),
        volume24h: 0, // Not available without indexer
        volume: totalPoolSol,
        liquidity: totalPoolSol,
        url: `https://baozi.bet/market/${pubkey}`,
        category: layerName(market.layer),
        tags: [layerName(market.layer), 'solana', 'pari-mutuel'],
    };
    (0, market_utils_1.addBinaryOutcomes)(um);
    return um;
}
function mapRaceToUnified(market, pubkey) {
    const totalPoolSol = Number(market.totalPool) / exports.LAMPORTS_PER_SOL;
    const outcomes = [];
    for (let i = 0; i < market.outcomeCount; i++) {
        const pool = market.outcomePools[i];
        // Implied probability: complement side pool / total pool
        const otherPools = market.totalPool - pool;
        const price = market.totalPool > 0n
            ? Number(otherPools) / (Number(market.totalPool) * (market.outcomeCount - 1))
            : 1 / market.outcomeCount;
        outcomes.push({
            outcomeId: `${pubkey}-${i}`,
            marketId: pubkey,
            label: market.outcomeLabels[i] || `Outcome ${i + 1}`,
            price: (0, price_1.clampBaoziPrice)(price),
        });
    }
    // Normalize prices to sum to 1
    (0, price_1.normalizeBaoziOutcomes)(outcomes);
    const um = {
        marketId: pubkey,
        title: market.question,
        description: '',
        outcomes,
        resolutionDate: new Date(Number(market.resolutionTime) * 1000),
        volume24h: 0,
        volume: totalPoolSol,
        liquidity: totalPoolSol,
        url: `https://baozi.bet/market/${pubkey}`,
        category: layerName(market.layer),
        tags: [layerName(market.layer), 'solana', 'pari-mutuel', 'race'],
    };
    // For 2-outcome races, add binary convenience getters
    (0, market_utils_1.addBinaryOutcomes)(um);
    return um;
}
// ---------------------------------------------------------------------------
// PDA Derivation
// ---------------------------------------------------------------------------
function deriveConfigPda() {
    const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('config')], exports.PROGRAM_ID);
    return pda;
}
function deriveMarketPda(marketId) {
    const buf = Buffer.alloc(8);
    buf.writeBigUInt64LE(marketId);
    const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('market'), buf], exports.PROGRAM_ID);
    return pda;
}
function derivePositionPda(marketId, user) {
    const buf = Buffer.alloc(8);
    buf.writeBigUInt64LE(marketId);
    const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('position'), buf, user.toBuffer()], exports.PROGRAM_ID);
    return pda;
}
function deriveRaceMarketPda(marketId) {
    const buf = Buffer.alloc(8);
    buf.writeBigUInt64LE(marketId);
    const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('race'), buf], exports.PROGRAM_ID);
    return pda;
}
function deriveRacePositionPda(marketId, user) {
    const buf = Buffer.alloc(8);
    buf.writeBigUInt64LE(marketId);
    const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('race_position'), buf, user.toBuffer()], exports.PROGRAM_ID);
    return pda;
}
// ---------------------------------------------------------------------------
// Instruction Discriminators
// ---------------------------------------------------------------------------
function getInstructionDiscriminator(name) {
    return (0, crypto_1.createHash)('sha256').update(`global:${name}`).digest().subarray(0, 8);
}
exports.PLACE_BET_SOL_DISCRIMINATOR = getInstructionDiscriminator('place_bet_sol');
exports.BET_ON_RACE_OUTCOME_SOL_DISCRIMINATOR = getInstructionDiscriminator('bet_on_race_outcome_sol');
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function layerName(layer) {
    switch (layer) {
        case 0: return 'official';
        case 1: return 'lab';
        case 2: return 'private';
        default: return 'unknown';
    }
}
/** Simple TTL cache */
class Cache {
    data = null;
    timestamp = 0;
    ttlMs;
    constructor(ttlMs) {
        this.ttlMs = ttlMs;
    }
    get() {
        if (this.data && Date.now() - this.timestamp < this.ttlMs) {
            return this.data;
        }
        return null;
    }
    set(value) {
        this.data = value;
        this.timestamp = Date.now();
    }
    clear() {
        this.data = null;
        this.timestamp = 0;
    }
}
exports.Cache = Cache;
